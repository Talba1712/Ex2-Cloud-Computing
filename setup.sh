...
ssh -i $KEY_PEM -o "StrictHostKeyChecking=no" -o "ConnectionAttempts=60" ubuntu@$PUBLIC_IP1 <<EOF
    ...
    pip install boto3
    # run app
    nohup python3 app.py $PUBLIC_IP1 $PUBLIC_IP2 $ACCESS_KEY_ID $SECRET_ACCESS_KEY $KEY_NAME $SEC_GRP $ &>/dev/null &
...
ssh -i $KEY_PEM -o "StrictHostKeyChecking=no" -o "ConnectionAttempts=60" ubuntu@$PUBLIC_IP2 <<EOF
    ...
    pip install boto3
    # run app
    nohup python3 app.py $PUBLIC_IP2 $PUBLIC_IP1 $ACCESS_KEY_ID $SECRET_ACCESS_KEY $KEY_NAME $SEC_GRP &>/dev/null &
